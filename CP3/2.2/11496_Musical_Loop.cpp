...
int main( int argc, char ** argv ) {
    ...
    while ( true ) {
        ...
        vector< int > samplesVec( N, 0 );
        for ( int n = 0; n < N; n++ ) {
            cin >> samplesVec[ n ];
        }

        samplesVec.push_back( samplesVec[ 0 ] );
        samplesVec.push_back( samplesVec[ 1 % samplesVec.size() ] );

        int samplesCount = 0;
        for ( int i = 1; i < samplesVec.size() - 1; i++ ) {
            if ( ( samplesVec[ i ] - samplesVec[ i - 1 ] ) * ( samplesVec[ i + 1 ] - samplesVec[ i ] ) < 0 ) {
                ++samplesCount;
            }
        }
        cout << samplesCount << endl;
    }
