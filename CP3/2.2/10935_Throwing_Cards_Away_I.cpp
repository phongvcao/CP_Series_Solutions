...
// Solution 1:
//
// - We need a deque< int > of size N & fill it up with values from 1 to N
// - while the size of the deque > 1, keep simulation running
//
// Time Complexity : O( N )
...
int main( int argc, char ** argv ) {
    ...
    while ( cin >> numCards ) {
        ...
        deque< int > cardsDeq( numCards, 0 );
        iota( cardsDeq.begin(), cardsDeq.end(), 1 );

        cout << "Discarded cards:";
        while ( cardsDeq.size() > 1 ) {
            if ( cardsDeq.size() > 2 ) {
                cout << " " << cardsDeq.front() << ",";
            } else {
                cout << " " << cardsDeq.front();
            ...
            cardsDeq.pop_front();
            cardsDeq.push_back( cardsDeq.front() );
            cardsDeq.pop_front();
        }
        cout << endl;

        cout << "Remaining card: " << cardsDeq.front() << endl;
    }
