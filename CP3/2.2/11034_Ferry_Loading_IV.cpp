...
// Solution 1:
//
// - We need a vector< queue< int > > that represents the 2 left & right queues
// for the cars
//
// - As we're at bank curBankIndex, we try to fit as many cars on the ferry as
// we can ( carsOnFerryQueue ). Then we ++riverCrossesCount every time the
// totalCarsLength reaches ferryLength.
//
...
int main( int argc, char ** argv ) {
    ...
    for ( int c = 0; c < C; c++ ) {
        vector< queue< int > > banksQueueVec( 2, queue< int >() );
        ...
        for ( int i = 0; i < numCars; i++ ) {
            ...
            cin >> carLength >> bankName;

            banksQueueVec[ Solution::BANK_NAMES_TO_INDICES[ bankName ] ].push( carLength );
        }

        int passesCount = 0;
        int carsOnFerryLength = 0;
        int curBankIndex = 0;
        while ( true ) {
            ...
            // Take as many cars as possible from current bank
            while ( ! banksQueueVec[ curBankIndex ].empty()
                    && carsOnFerryLength + banksQueueVec[ curBankIndex ].front() <= ferryLength ) {
                carsOnFerryLength += banksQueueVec[ curBankIndex ].front();
                banksQueueVec[ curBankIndex ].pop();
            }

            // Cross the river
            if ( carsOnFerryLength > 0 ) {
                ++passesCount;
                carsOnFerryLength = 0;
            }

            // If the other bank is not empty, we switch bank
            if ( ! banksQueueVec[ curBankIndex ^ 1 ].empty() ) {
                curBankIndex ^= 1;
            } else if ( ! banksQueueVec[ curBankIndex ].empty() ) {
                // Otherwise, if the other bank is empty, we come bank to current
                // bank
                ++passesCount;
            }
        }

        cout << passesCount << endl;
    }
