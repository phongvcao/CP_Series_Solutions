...
int main( int argc, char ** argv ) {
    ...
    while ( true ) {
        ...
        for ( int n = 0; n < N; n++ ) {
            if ( getline( cin, line ) ) {
                int lineIdx = 0;
                while ( lineIdx < line.size() && line[ lineIdx++ ] == 'X' ) {
                    ++nonVoidVec[ n ];
                }

                if ( lineIdx < line.size() ) {
                    lineIdx = line.size() - 1;
                    while ( lineIdx >= 0 && line[ lineIdx-- ] == 'X' ) {
                        ++nonVoidVec[ n ];
                    }
                }

                if ( nonVoidVec[ n ] > maxNonVoid ) {
                    maxNonVoid = nonVoidVec[ n ];
                }
                totalNonVoid += nonVoidVec[ n ];

            }
        }

        cout << maxNonVoid * nonVoidVec.size() - totalNonVoid << endl;
