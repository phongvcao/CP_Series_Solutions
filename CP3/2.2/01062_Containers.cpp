...
int main( int argc, char ** argv ) {
    ...
    while ( cin >> line ) {
        ...
        map< char, stack< char > > stackMap;

        for ( int i = 0; i < line.size(); i++ ) {
            auto iter = stackMap.find( line[ i ] );
            if ( iter != stackMap.end() ) {
                iter->second.push( line[ i ] );
            } else {
                iter = stackMap.lower_bound( line[ i ] );

                if ( iter != stackMap.end() ) {
                    stack< char > charStack = iter->second;
                    char stackTop = charStack.top();

                    charStack.push( line[ i ] );
                    stackMap.erase( stackTop );
                    stackMap.insert( { line[ i ], charStack } );
                } else {
        ...
        cout << "Case " << caseCount++ << ": " << stackMap.size() << endl;
    }
