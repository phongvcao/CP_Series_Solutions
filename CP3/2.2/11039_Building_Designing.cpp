...
int main( int argc, char ** argv ) {
    ...
    for ( int t = 0; t < T; t++ ) {
        ...
        vector< int > floorsVec( numFloors, 0 );
        for ( int i = 0; i < numFloors; i++ ) {
            cin >> floorsVec[ i ];
        }

        sort( floorsVec.begin(), floorsVec.end(), cmpFloorsByAbsValues() );

        int negCount = 0;
        int posCount = 0;
        int currentSign = floorsVec[ 0 ] > 0 ? 1 : -1;
        if ( currentSign < 0 ) {
            ++negCount;
        } else {
            ++posCount;
        ...
        for ( int i = 1; i < floorsVec.size(); i++ ) {
            if ( floorsVec[ i ] * currentSign < 0 ) {
                if ( floorsVec[ i ] < 0 ) {
                    ++negCount;
                } else {
                    ++posCount;
        ...
        int floorsCount = 2 * min( posCount, negCount );
        if ( posCount != negCount ) {
            ++floorsCount;
        }

        if ( floorsCount == 0 ) ++ floorsCount;
        cout << floorsCount << endl;
