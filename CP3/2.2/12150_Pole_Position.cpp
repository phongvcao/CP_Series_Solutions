...
int main( int argc, char ** argv ) {
    ...
    while ( true ) {
        ...
        vector< int > positionsVec( N, -1 );

        bool answerFound = false;
        for ( int n = 0; n < N; n++ ) {
            int carNum = 0;
            int posGainedLost = 0;
            cin >> carNum >> posGainedLost;
            if ( ! answerFound ) {
                if ( n + posGainedLost < 0 || n + posGainedLost > N || positionsVec[ n + posGainedLost ] != -1 ) {
                    cout << -1 << endl;
                    answerFound = true;
                } else {
                    positionsVec[ n + posGainedLost ] = carNum;
                }
            }
        }
