...
int main( int argc, char ** argv ) {
    ...
    for ( int n = 0; n < N; n++ ) {
        long long numCookies = 0;
        long long numLeft = 0;
        cin >> numCookies >> numLeft;

        set< long long > cookiesPerGuestSet;
        for ( long long i = 1; i < ( long long )sqrt( numCookies - numLeft ) + 1; i++ ) {
            if ( ( numCookies - numLeft ) % i == 0 ) {
                if ( numLeft < i ) {
                    cookiesPerGuestSet.insert( i );
                }

                if ( numLeft < ( numCookies - numLeft ) / i ) {
                    cookiesPerGuestSet.insert( ( numCookies - numLeft ) / i );
                }
            }
        }

        cout << "Case #" << n + 1 << ":";
        if ( numCookies == numLeft ) {
            cout << " " << 0;
        } else {
            for ( long long cookiesPerGuest : cookiesPerGuestSet ) {
                cout << " " << cookiesPerGuest;
            }
        }
        cout << endl;
    }
