...
int main( int argc, char ** argv ) {
    ...
    while ( getline( cin, line ) ) {
        ...
        while ( lineIdx < line.size() ) {
            if ( isalpha( line[ lineIdx ] ) ) {
                if ( newStr.empty() ) {
                    newStr += line[ lineIdx ];
                } else {
                    if ( Soundex::LETTERS_TO_DIGITS[ lastChar - 'A' ] != Soundex::LETTERS_TO_DIGITS[ line[ lineIdx ] - 'A' ]
                            && Soundex::LETTERS_TO_DIGITS[ line[ lineIdx ] - 'A' ] != -1 ) {
                        newStr += line[ lineIdx ];
                    }
                }
                lastChar = line[ lineIdx ];
        ...
        for ( int i = 0; i < newStr.size(); i++ ) {
            if ( Soundex::LETTERS_TO_DIGITS[ newStr[ i ] - 'A' ] != -1 ) {
                cout << Soundex::LETTERS_TO_DIGITS[ newStr[ i ] - 'A' ];
            }
        }
        cout << endl;
    }
