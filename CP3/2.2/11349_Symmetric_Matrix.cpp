...
int main( int argc, char ** argv ) {
    ...
    //
    // Symmetric matrix is such a matrix that:
    //   - All elements of it are non-negative
    //   - All elements of it are symmetric with relation to the center of this
    //   matrix.
    //
    ...
    for ( int t = 0; t < T; t++ ) {
        ...
        cin >> ignoreStr1 >> ignoreStr2 >> N;

        bool isSymmetric = true;
        vector< vector< long long > > matrixVec( N, vector< long long >( N, -1 ) );
        for ( int i = 0; i < N; i++ ) {
            for ( int j = 0; j < N; j++ ) {
                cin >> matrixVec[ i ][ j ];
        ...
        if ( ! isSymmetric ) {
            ...
            // Now we go through the upper part of the matrix to calculate
            // symmetry
            for ( int i = 0; i < ( matrixVec.size() + 1 ) / 2; i++ ) {
                for ( int j = 0; j < matrixVec[ i ].size(); j++ ) {
                    if ( matrixVec[ i ][ j ] != matrixVec[ matrixVec.size() - 1 - i ][ matrixVec[ i ].size() - 1 - j ] ) {
                        isSymmetric = false;
                        break;
                    }
                }
