...
int main( int argc, char ** argv ) {
    ...
    for ( int t = 0; t < T; t++ ) {
        ...
        vector< int > isDayLostVec( numDays, false );
        ...
        for ( int i = 0; i < numParties; i++ ) {
            ...
            for ( int j = hartalParam - 1; j < numDays; j += hartalParam ) {
                if ( j % Hartals::NUM_WEEK_DAYS != Hartals::WEEK_FRIDAY && j % Hartals::NUM_WEEK_DAYS != Hartals::WEEK_SATURDAY ) {
                    if ( ! isDayLostVec[ j ] ) {
                        ++daysLostCount;
                    }
                    isDayLostVec[ j ] = true;
