...
int main( int argc, char ** argv ) {
    ...
    while ( true ) {
        ...
        int numCols = 0;
        int numSetters = 0;
        cin >> numRows >> numCols >> numSetters;

        if ( numRows == 0 && numCols == 0 && numSetters == 0 ) {
            break;
        }

        vector< bitset< 10000 > > rowsVec( numRows, bitset< 10000 >().set() );
        vector< bitset< 10000 > > colsVec( numCols, bitset< 10000 >().set() );
        ...
        if ( rowsVec[ row ][ col ] && colsVec[ col ][ row ] ) {
            isEscapable = true;
        }

        if ( row > 0 ) {
            if ( rowsVec[ row - 1 ][ col ] && colsVec[ col ][ row - 1 ] ) {
                isEscapable = true;
            }
        }

        if ( col > 0 ) {
            if ( rowsVec[ row ][ col - 1 ] && colsVec[ col - 1 ][ row ] ) {
                isEscapable = true;
            }
        }
