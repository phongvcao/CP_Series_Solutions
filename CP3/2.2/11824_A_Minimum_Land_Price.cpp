...
namespace Land {
    static const int MAX_LAND_PRICE = 5000000;
}
...
int main( int argc, char ** argv ) {
    ...
    for ( int t = 0; t < T; t++ ) {
        int landCost = 0;
        vector< int > landCostVec;
        do {
            cin >> landCost;
            landCostVec.push_back( landCost );
        } while ( landCost != 0 );

        sort( landCostVec.begin(), landCostVec.end(), greater< int >() );

        int totalCost = 0;
        int yearNum = 1;
        for ( int i = 0; i < landCostVec.size(); i++ ) {
            totalCost += 2 * pow( landCostVec[ i ], yearNum++ );
            if ( totalCost > Land::MAX_LAND_PRICE ) {
                break;
            }
        }
