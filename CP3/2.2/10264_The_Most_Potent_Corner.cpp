...
int main( int argc, char ** argv ) {
    ...
    while ( true ) {
        ...
        vector< int > cornerWeightsVec( ( int )pow( 2, N ), 0 );
        for ( int i = 0; i < ( int )pow( 2, N ); i++ ) {
            cin >> cornerWeightsVec[ i ];
        }

        vector< int > potenciesSumVec( cornerWeightsVec.size(), 0 );
        for ( int i = 0; i < cornerWeightsVec.size(); i++ ) {
            int potenciesSum = 0;
            for ( int j = 0; j < N; j++ ) {
                int index = i ^ ( 1 << j );
                potenciesSumVec[ i ] += cornerWeightsVec[ index ];
            }
        }

        int maxPotenciesSum = 0;
        for ( int i = 0; i < potenciesSumVec.size(); i++ ) {
            for ( int j = 0; j < N; j++ ) {
                int index = i ^ ( 1 << j );
                if ( potenciesSumVec[ i ] + potenciesSumVec[ index ] > maxPotenciesSum ) {
                    maxPotenciesSum = potenciesSumVec[ i ] + potenciesSumVec[ index ];
                }
            }
        }

        cout << maxPotenciesSum << endl;
    }
