...
int main( int argc, char ** argv ) {
    ...
    while ( true ) {
        int numSingleTasks = 0;
        int numRepeatedTasks = 0;
        cin >> numSingleTasks >> numRepeatedTasks;

        if ( numSingleTasks == 0 && numRepeatedTasks == 0 ) {
            break;
        }

        bitset< MAX_TIME > timeBitSet;
        bool isConflict = false;
        for ( int i = 0; i < numSingleTasks; i++ ) {
            int startTime = 0;
            int endTime = 0;
        ...
        for ( int i = 0; i < numRepeatedTasks; i++ ) {
            int startTime = 0;
            int endTime = 0;
            int interval = 0;
            ...
            if ( ! isConflict ) {
                for ( int j = 0; j < MAX_TIME; j += interval ) {
                    for ( int k = startTime; k < endTime && j + k < MAX_TIME; k++ ) {
                        if ( timeBitSet[ j + k ] ) {
                            isConflict = true;
                            break;
                        }
