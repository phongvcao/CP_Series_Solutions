...
int main( int argc, char ** argv ) {
    ...
    while ( true ) {
        ...
        vector< int > stationsVec( N );
        for ( int n = 0; n < N; n++ ) {
            cin >> stationsVec[ n ];
        }

        sort( stationsVec.begin(), stationsVec.end() );

        if ( stationsVec[ 0 ] > 200 ) {
            cout << "IMPOSSIBLE" << endl;
        } else {
            bool answerFound = false;
            for ( int i = 0; i < stationsVec.size() - 1; i++ ) {
                if ( stationsVec[ i + 1 ] - stationsVec[ i ] > 200 ) {
                    cout << "IMPOSSIBLE" << endl;
                    answerFound = true;
                    break;
                }
