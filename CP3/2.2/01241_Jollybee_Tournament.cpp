...
int main( int argc, char ** argv ) {
    ...
    for ( int t = 0; t < T; t++ ) {
        ...
        deque< int > participationDeque( ( int )pow( 2, N ), 1 );

        for ( int i = 0; i < M; i++ ) {
            int playerNum = 0;
            cin >> playerNum;

            participationDeque[ playerNum - 1 ] = 0;
        }

        int walkOverCount = 0;
        while ( participationDeque.size() > 1 ) {
            int player1 = participationDeque.front();
            participationDeque.pop_front();
            int player2 = participationDeque.front();
            participationDeque.pop_front();

            walkOverCount += player1 ^ player2;
            participationDeque.push_back( player1 | player2 );
        }
        cout << walkOverCount << endl;
    }
