...
int main( int argc, char ** argv ) {
    ...
    for ( int t = 0; t < T; t++ ) {
        int numRows = 0;
        int numCols = 0;
        int importantCellBytes = 0;
        int normalCellBytes = 0;
        cin >> numRows >> numCols >> importantCellBytes >> normalCellBytes;

        vector< pair< int, int > > cellsCountVec( 26, make_pair( 0, 0 ) );
        for ( int i = 0; i < cellsCountVec.size(); i++ ) {
            cellsCountVec[ i ].first = i;
        }
        ...
        for ( int i = 0; i < numRows; i++ ) {
            ...
            for ( int j = 0; j < line.size(); j++ ) {
                ++cellsCountVec[ line[ j ] - 'A' ].second;
            }
        }

        sort( cellsCountVec.begin(), cellsCountVec.end(), cmpCellsByCount() );

        int maxCellsCount = cellsCountVec.begin()->second;
        int totalBytes = 0;
        for ( int i = 0; i < cellsCountVec.size(); i++ ) {
            if ( cellsCountVec[ i ].second == maxCellsCount ) {
                totalBytes += maxCellsCount * importantCellBytes;
            } else {
                totalBytes += cellsCountVec[ i ].second * normalCellBytes;
            }
        }

        cout << "Case " << t + 1 << ": " << totalBytes << endl;
    }
