...
namespace Grid {
    static bool isFinite( const vector< vector< int > >& matrixVec ) {
        for ( int i = 0; i < matrixVec.size(); i++ ) {
            for ( int j = 0; j < matrixVec[ i ].size(); j++ ) {
                if ( matrixVec[ i ][ j ] != 0 ) return true;
            }
        }
        return false;
    }
...
int main( int argc, char ** argv ) {
    ...
    for ( int n = 0; n < N; n++ ) {
        vector< vector< int > > matrixVec( 3, vector< int >( 3, 0 ) );

        for ( int i = 0; i < 3; i++ ) {
            string line = "";
            cin >> line;
            for ( int j = 0; j < line.size(); j++ ) {
                matrixVec[ i ][ j ] = line[ j ] - '0';
            }
        }

        int index = -1;
        while ( Grid::isFinite( matrixVec ) ) {
            ++index;
            ...
            for ( int i = 0; i < matrixVec.size(); i++ ) {
                for ( int j = 0; j < matrixVec[ i ].size(); j++ ) {
                    ...
                    if ( i > 0 ) {
                        sum += orgMatrixVec[ i - 1 ][ j ];
                    }

                    if ( j > 0 ) {
                        sum += orgMatrixVec[ i ][ j - 1 ];
                    }

                    if ( i < orgMatrixVec.size() - 1 ) {
                        sum += orgMatrixVec[ i + 1 ][ j ];
                    }

                    if ( j < orgMatrixVec.size() - 1 ) {
                        sum += orgMatrixVec[ i ][ j + 1 ];
                    }
