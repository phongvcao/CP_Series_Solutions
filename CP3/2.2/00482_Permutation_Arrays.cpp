...
int main( int argc, char ** argv ) {
    ...
    for ( int n = 0; n < N; n++ ) {
        ...
        while ( getline( cin, line ) ) {
            ...
            string numStr = "";
            while ( ss >> numStr ) {
                numbersVec.push_back( numStr );
            }
            break;
        }

        map< int, string > numbersMap;
        for ( int i = 0; i < indexesVec.size(); i++ ) {
            numbersMap.insert( { indexesVec[ i ], numbersVec[ i ] } );
        }

        for ( map< int, string >::iterator iter = numbersMap.begin(); iter != numbersMap.end(); iter++ ) {
