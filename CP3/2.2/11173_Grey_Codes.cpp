...
int main( int argc, char ** argv ) {
    ...
    for ( int n = 0; n < N; n++ ) {
        ...
        int initialNum = 0;
        int bitsCount = numBits;
        bool maybeXor = false;

        while ( bitsCount > 0 ) {
            int lastBit = initialNum & 0x1;
            initialNum <<= 1;
            if ( ( double )( k + 1 ) / ( double )pow( 2, bitsCount ) <= 0.5 ) {
                // Upper half
                initialNum |= 0x0;
                if ( maybeXor ) initialNum ^= 0x1;
                maybeXor = false;
            } else {
                initialNum |= 0x1;
                if ( maybeXor ) initialNum ^= 0x1;
            ...
            bitsCount -= 1;
            if ( k >= pow( 2, bitsCount ) ) {
                k -= pow( 2, bitsCount );
            }
        }

        cout << initialNum << endl;
    }
