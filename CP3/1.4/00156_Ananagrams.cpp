...
int main( int argc, char ** argv ) {
    ...
    while ( getline( cin, line ) ) {
        ...
        while ( ss >> word ) {
            ...
            if ( anagramsMap.find( lowerStr ) != anagramsMap.end() ) {
                anagramsMap[ lowerStr ].push_back( word );
            } else {
                anagramsMap.insert( { lowerStr, { word } } );
            }

        }
    }

    set< string > sortedOutputSet;
    for ( unordered_map< string, vector< string > >::iterator iter = anagramsMap.begin();
            iter != anagramsMap.end(); ++iter ) {
        if ( iter->second.size() == 1 ) {
            sortedOutputSet.insert( iter->second[ 0 ] );
        }
    }

    for ( const string& s : sortedOutputSet ) {
        cout << s << endl;
    }
