...
int main( int argc, char ** argv ) {
    ...
    for ( int t = 0; t < T; t++ ) {
        int keyPressesCount = 0;
        if ( getline( cin, line ) ) {
            for ( int i = 0; i < line.size(); i++ ) {
                if ( line[ i ] == ' ' ) {
                    keyPressesCount += KEY_PRESSES_VEC.back();
                } else {
                    keyPressesCount += KEY_PRESSES_VEC[ line[ i ] - 'a' ];
                }
            }
        }
        cout << "Case #" << t + 1 << ": " << keyPressesCount << endl;
    }
