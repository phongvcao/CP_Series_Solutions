...
int main( int argc, char ** argv ) {
    ...
    // We have to divide the lost seconds by 24 * 60
    while ( getline( cin, line ) ) {
        long long firstWatchLoss = 0;
        long long secondWatchLoss = 0;

        stringstream ss( line );
        ss >> firstWatchLoss >> secondWatchLoss;

        long long diffLoss = abs( firstWatchLoss - secondWatchLoss );
		 ...
		 */
        long long secs = ( 43200 * ( 86400 - firstWatchLoss ) / diffLoss ) % 43200 + 30;

        long long hrs = secs / 3600 % 12;
        long long mins = secs / 60 % 60;

        if ( hrs == 0 ) hrs = 12;

        printf( "%lld %lld %02lld:%02lld\n", firstWatchLoss, secondWatchLoss, hrs, mins );
    }
