...
int main( int argc, char ** argv ) {
    ...
    for ( int t = 0; t < T; t++ ) {
        ...
        for ( int n = 0; n < N; n++ ) {
            if ( getline( cin, line ) ) {
                string anaKey = line;
                sort( anaKey.begin(), anaKey.end() );

                if ( anaDict.find( anaKey ) != anaDict.end() ) {
                    anaDict[ anaKey ].push_back( line );
                } else {
                    anaDict.insert( { anaKey, { line } } );
                }
            }
        }
        ...
        while ( getline( cin, line ) ) {
            ...
            cout << "Anagrams for: " << line << endl;
            if ( anaDict.find( anaKey ) != anaDict.end() ) {
                int index = 1;
                for ( const string& s : anaDict[ anaKey ] ) {
                    cout << "  " << index << ") " << s << endl;
                    ++index;
                }
            } else {
                cout << "No anagrams for: " << line << endl;
            }
        }
