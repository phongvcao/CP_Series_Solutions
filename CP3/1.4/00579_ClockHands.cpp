...
namespace ClockHands {
    static const vector< int > HOURS_TO_MINUTES_HANDS = {
...
int main( int argc, char ** argv ) {
    ...
    while ( getline( cin, line ) ) {
        ...
        int colonIdx = line.find_first_of( ":" );
        int hour = atoi( line.substr( 0, colonIdx ).c_str() );
        int minute = atoi( line.substr( colonIdx + 1 ).c_str() );

        double deg = ( double )( ClockHands::HOURS_TO_MINUTES_HANDS[ hour ] - minute ) * 360.0 / 60.0;

        if ( deg < 0 ) {
            deg = 360.0 + deg;
        }
        deg += ( double )minute / 60.0 * 30;
