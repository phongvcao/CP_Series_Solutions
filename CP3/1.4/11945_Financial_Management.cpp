...
int main( int argc, char ** argv ) {
    ...
    for ( int n = 0; n < N; n++ ) {
        double sum = 0;
        int monthCount = 0;
        while ( monthCount < 12 && getline( cin, line ) ) {
        ...
        double finalResult = round( sum * 100.0 ) / 1200.0;

        vector< int > partsVec;
        int decimal = ( int )( round( ( finalResult - floor( finalResult ) ) * 100.0 ) );

        while ( finalResult > 1 ) {
            int part = ( int )( finalResult - ( double )( ( int )( finalResult / 1000.0 ) * 1000 ) );
        ...
        if ( partsVec.empty() ) {
            ...
            for ( int i = partsVec.size() - 1; i >= 0; i-- ) {
                if ( i == partsVec.size() - 1 ) {
                    printf( "%d", partsVec[ i ] );
                } else {
                    printf( ",%03d", partsVec[ i ] );
                }
            }
            printf( ".%02d\n", decimal );
        }
